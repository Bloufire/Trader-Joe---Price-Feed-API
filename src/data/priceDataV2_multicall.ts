...
import { JOE_FACTORY_V2 as JOE_FACTORY, MIN_LIQUIDITY, MULTICALL_CONTRACT } from '../constants';
import { baseWeb3 } from '../utils/web3';
import { getCache, setCache } from '../utils/cache';
import { Bin, LBFactoryABI, LBPairABI } from '@traderjoe-xyz/sdk-v2';
...
export const getPrice = async (base_asset: string, quote_asset: string, bin_step: string, checkLiquidity: boolean) => {
    ...
    // Get tokenX, tokenY and activeBinId
    const pairContract = getContract(LBPairABI, pairAddress, baseWeb3());
    const tokenXCalls = [
        { tokenX: pairContract.methods.tokenX() },
    ];
    const tokenYCalls = [
        { tokenY: pairContract.methods.tokenY() },
    ];
    const getReservesAndIdCalls = [
        { reserveAndId: pairContract.methods.getReservesAndId() },
    ];
    const assets_multicall = await multicall.all([tokenXCalls, tokenYCalls, getReservesAndIdCalls]);
    const tokenX = assets_multicall[0][0].tokenX;
    const tokenY = assets_multicall[1][0].tokenY;
    const activeBinId = assets_multicall[2][0].reserveAndId[2] as number;
...
export const fetchData = async (base_asset: string, quote_asset: string, bin_step: string, checkLiquidity: boolean = true) => {
    const cacheKey = `single_v2:${base_asset}_${quote_asset}`;
    const invertedCacheKey = `single_v2:${quote_asset}_${base_asset}`;
